...
void GenIR::visit(DefAST &ast) {
  ...
  if (scope.in_global()) {
    if (ast.arrays.empty()) {       // 不是数组，即全局量
      if (ast.initVal == nullptr) { // 无初始化
        if (isConst)
          cout << "no initVal when define const!"
               << endl; // 无初始化全局常量报错
        // 无初始化全局量一定是变量
        GlobalVariable *var;
        if (curType == INT32_T)
          var = new GlobalVariable(varName, module.get(), curType, false,
                                   CONST_INT(0));
        else
          var = new GlobalVariable(varName, module.get(), curType, false,
                                   CONST_FLOAT(0));
      ...
      vector<ArrayType *> arrayTys(
          dimensions.size()); // 数组类型, {[2 x [3 x [4 x i32]]], [3 x [4 x
                              // i32]], [4 x i32]}
      ...
      if (ast.initVal == nullptr || ast.initVal->initValList.empty()) {
        ...
        auto var = new GlobalVariable(varName, module.get(), arrayTys[0],
                                      isConst, init);
        scope.push(varName, var);
      } else {
        useConst = true; // 全局数组量的初始值必为常量
        auto init =
            globalInit(dimensions, arrayTys, 0, ast.initVal->initValList);
        useConst = false;
        auto var = new GlobalVariable(varName, module.get(), arrayTys[0],
                                      isConst, init);
        scope.push(varName, var);
  ...
  if (ast.arrays.empty()) {       // 不是数组，即普通局部量
    ...
    vector<int> dimensions(ast.arrays.size()),
        dimensionsCnt((ast.arrays.size())); // 数组各维度, [2][3][4]对应;
                                            // 次维度数组元素个数, [24][12][4]
    ...
    totalByte *= 4; // 计算字节数
    useConst = false;
    ArrayType *arrayTy = nullptr; // 数组类型
    for (int i = dimensions.size() - 1; i >= 0; i--) {
      if (i == dimensions.size() - 1)
        arrayTy = module->get_array_type(curType, dimensions[i]);
      else
    ...
    builder->create_call(
        memclr, {i32P, CONST_INT(totalByte)}); // 全部清零，但float可以清零吗
    // 数组初始化时，成员exp一定是空，若initValList也是空，即是大括号，已经置零了直接返回
    if (ast.initVal->initValList.empty())
      return;
...
// 嵌套大括号数组的维度，即倒数连续0的第一个。
// 如[0,1,0,0]，返回2；[0,0,0,1]，返回4； 若全是0，[0,0,0,0],返回1
...
void GenIR::mergeElements(vector<int> &dimensions,
...
void GenIR::finalMerge(vector<int> &dimensions, vector<ArrayType *> &arrayTys,
...
// 生成变量数组的初始化
ConstantArray *GenIR::globalInit(vector<int> &dimensions,
...
void GenIR::localInit(Value *ptr, vector<unique_ptr<InitValAST>> &list,
  ...
  for (auto &initVal : list) {
    if (initVal->exp) {
      ...
      if (!initVal->initValList.empty()) {
        if (cnt != 0)
          tempPtr = builder->create_gep(
              ptr, {CONST_INT(cnt)}); // 没赋值过，那tempPtr实际就是ptr
...
void GenIR::visit(FuncDefAST &ast) {
  ...
  paramNames.clear();
  Type *retType;
  if (ast.funcType == TYPE_INT)
    retType = INT32_T;
  else if (ast.funcType == TYPE_FLOAT)
    retType = FLOAT_T;
  else
    retType = VOID_T;
  ...
  for (auto arg = func->arguments_.begin(); arg != func->arguments_.end();
       arg++)
    args.push_back(*arg);
...
void GenIR::visit(ReturnStmtAST &ast) {
  ...
  has_br = true;
  // builder->BB_ = new BasicBlock(module.get(), to_string(id++),
  // currentFunction); //return语句后必是新块
}
...
void GenIR::visit(SelectStmtAST &ast) {
  ...
  if (ast.elseStmt == nullptr)
    ...
    nextIf = new BasicBlock(module.get(), to_string(id++), currentFunction);
...
void GenIR::visit(IterationStmtAST &ast) {
  ...
  auto tempWhileFalseBB =
      whileFalseBB; // break只跳while的false，而不跳全局false
...
bool GenIR::checkCalType(Value *val[], int intVal[], float floatVal[]) {
  ...
  if (dynamic_cast<ConstantInt *>(val[0]) &&
      dynamic_cast<ConstantInt *>(val[1])) {
    resultIsInt = true;
    intVal[0] = dynamic_cast<ConstantInt *>(val[0])->value_;
    intVal[1] = dynamic_cast<ConstantInt *>(val[1])->value_;
  } else { // 操作结果一定是float
    if (dynamic_cast<ConstantInt *>(val[0]))
      floatVal[0] = dynamic_cast<ConstantInt *>(val[0])->value_;
    else
      floatVal[0] = dynamic_cast<ConstantFloat *>(val[0])->value_;
    if (dynamic_cast<ConstantInt *>(val[1]))
      floatVal[1] = dynamic_cast<ConstantInt *>(val[1])->value_;
    else
      floatVal[1] = dynamic_cast<ConstantFloat *>(val[1])->value_;
  }
  return resultIsInt;
}
...
void GenIR::visit(AddExpAST &ast) {
  ...
  if (useConst) {
    ...
    if (resultIsInt)
      recentVal = CONST_INT(intVal[2]);
    else
      recentVal = CONST_FLOAT(floatVal[2]);
    return;
  }
...
void GenIR::visit(LValAST &ast) {
  ...
  if (scope.in_global()) {
    ...
    recentVal =
        ((GlobalVariable *)var)->init_val_; // 使用var的初始化数组查找常量元素
    for (int i : index) {
      ...
      if (dynamic_cast<ConstantZero *>(recentVal)) {
        ...
        if (arrayTy == INT32_T)
          recentVal = CONST_INT(0);
        else
          recentVal = CONST_FLOAT(0);
        return;
      }
  ...
  // 局部作用域
  if (var->type_->tid_ == Type::IntegerTyID ||
      var->type_->tid_ == Type::FloatTyID) { // 说明为局部常量
    recentVal = var;
    return;
  }
  // 不是常量那么var一定是指针类型
  Type *varType =
      static_cast<PointerType *>(var->type_)->contained_; // 所指的类型
...
void GenIR::visit(NumberAST &ast) {
  if (ast.isInt)
    ...
    recentVal = CONST_FLOAT(ast.floatval);
}
  ...
  for (int i = 0; i < ast.funcCParamList.size(); i++) {
    ...
    if (recentVal->type_ == INT32_T && fun->arguments_[i]->type_ == FLOAT_T) {
      recentVal = builder->create_sitofp(recentVal, FLOAT_T);
               ...
               fun->arguments_[i]->type_ == INT32_T) {
      recentVal = builder->create_fptosi(recentVal, INT32_T);
    }
